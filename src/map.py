...
def printMap(V):
    ...
    for i in range(map.shape[0]):
        for j in range(map.shape[1]):
            if(map[i][j].split(':')[0] == pt.BLANK):
                ...
                wall_colour = Back.BLACK
                if V.wall_objs[(i,j)].level == 1:
                    wall_colour = Back.LIGHTBLACK_EX
                elif V.wall_objs[(i,j)].level == 3:
                    wall_colour = Back.LIGHTYELLOW_EX
                else:
                    wall_colour = Back.LIGHTRED_EX

                map_matrix[2*i][2*j] = wall_colour + '  '
                map_matrix[2*i][2*j+1] = wall_colour + '  '
                map_matrix[2*i+1][2*j] = wall_colour + '  '
                map_matrix[2*i+1][2*j+1] = wall_colour + '  '
                ...
                map_matrix[2*i+3][2*j] = color + str(V.cannon_objs[(i,j)].level) + ' '
                ...
                map_matrix[2*i+1][2*j] = color + str(V.wizard_tower_objs[(i,j)].level) + ' '
                map_matrix[2*i+1][2*j+1] = color + '  '
    ...
    for arch in archers:
        a= 2*arch.position[0]
        b = 2*arch.position[1]
        if(arch.isStealthArcher==True):
            map_matrix[a][b] = archColor(arch) + Fore.RED + 'ST' #1
            map_matrix[a+1][b] = archColor(arch) + Fore.RED + 'LT'#2
            map_matrix[a][b+1] = archColor(arch) + Fore.RED + 'EA' #3
            map_matrix[a+1][b+1] = archColor(arch) + Fore.RED + 'H ' #4
        else:
            map_matrix[a][b] = archColor(arch) + Fore.BLACK + 'AR' #1
            map_matrix[a+1][b] = archColor(arch) + Fore.BLACK + '  '#2
            map_matrix[a][b+1] = archColor(arch) + Fore.BLACK + 'CH' #3
            map_matrix[a+1][b+1] = archColor(arch) + Fore.BLACK + 'ER' #4
    ...
    for healer in healers:
        ...
        map_matrix[a][b] = hlColor(healer) + Fore.BLACK + 'HE'
        map_matrix[a+1][b] = hlColor(healer) + Fore.BLACK + '  '
        map_matrix[a][b+1] = hlColor(healer) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = hlColor(healer) + Fore.BLACK + 'ER'
