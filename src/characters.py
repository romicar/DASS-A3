...
class Archer:
    def __init__(self, position, stealthyyy=False):
        self.speed = 1
        self.health = 100
        self.max_health = 100
        self.attack = 1
        self.attack_radius = 3 # changed the attack radius 
        self.position = position
        self.alive = True
        self.target = None
        self.made = t.time() # this is the starting time 
        self.isStealthArcher =  stealthyyy# ok
    ...
    def checkforvisibility(self): # returns 1 if visible, 0 otherwise 
        if(self.isStealthArcher==False):
            return 1
        if(t.time()-self.made < 10):
            return 0
        else:
            return 1
...
class Healers:
    ...
    def insideAttackingRadius(self,r,c):
        if (r**2 + c**2 <= self.healrange**2):
            return True
        return False
    ...
    def move(self, pos,King):
        ...
        r = abs(pos[0] - self.position[0])
        c = abs(pos[1] - self.position[1])
        if (self.insideAttackingRadius(r,c)):
            self.heal(pos,King)
            return
        elif (r > c):
            if (pos[0] <= self.position[0]):
    ...
    def insideSplashRadius(self,r,c):
        if (max(r,c) <= self.healradius):
            return True
        return False

    def heal(self, pos, King):
        # troops = [barbarians, archers, balloons, dragons, [King]]
        for troops in [barbarians, archers, balloons, dragons]:
            for troop in troops:
                if troop.alive:
                    r = abs(pos[0] - troop.position[0])
                    c = abs(pos[1] - troop.position[1])
                    if self.insideSplashRadius(r,c):
...
def move_healers(King):
    for healer in healers:
        if(healer.alive == False):
            continue
        closest_soldier = search_for_closest_soldier(healer.position, King)
        if(closest_soldier == None):
            continue
        healer.move(closest_soldier, King)

def search_for_closest_soldier(pos, King):
    closest_soldier = None
    closest_distance = 10000000
    
    for troops in [barbarians, archers, dragons, balloons]:
        for troop in troops:
            if(troop.alive == True):
                if troop.health < troop.max_health:  
                    dist = abs(troop.position[0] - pos[0])
                    dist += abs(troop.position[1] - pos[1])
                    if dist >= closest_distance:
                        continue
                    closest_distance = min(dist,closest_distance)
                    closest_soldier = troop.position
    troop = King
    if(troop.alive == True):
        if troop.health < troop.max_health:  
            dist = abs(troop.position[0] - pos[0])
